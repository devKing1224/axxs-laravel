$(document).ready(function () {
    ...
    $('#familyAddDataSend').click(function () { 
        ...
        if (form.valid() === true) {
            var inmate_id = $('#inmate_id').val();//alert(inmate_id);return false;
            $.ajax({
                type: 'post',
                url: apiURL+'registerfamily',
                ...
                if (result.Code === 201) {
                    ...
                    window.location.href = baseURL+'familylist/'+inmate_id;
                    return false;
        ...
        swal({
            ...
            function(isConfirm){
                if (isConfirm) {   
                   $.ajax({
                        type: 'get',
                        url: apiURL+'deletefamily/'+familyID_inmateID[0],
                        dataType: 'json',
                        success: function (result) { 
                            if (result.Code === 200) {
                                sessionStorage.insert = 'Family deleted successfully';
                                window.location = baseURL+'familylist/'+familyID_inmateID[1];
                                return false;
                            } else if (result.Code === 400) {
                                alert(result.Message);
                                return false;
    ...
    $('#familyActiveInactiveCall').on('change', function(){
        ...
        if(familyID_inmateID[0] == 1){
           window.location.href =  baseURL+'familylist/'+familyID_inmateID[1];
        } else {
    ...
    $('.familyActiveButton').click(function(){
        ...
        $.ajax({
            type: 'post',
            data: { family_id : familyID_inmateID[0] },
            url: apiURL+'activefamily',
            dataType: 'json',
            success: function (result) { 
                if (result.Code === 200) {
