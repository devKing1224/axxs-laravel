          ...
          <div class="box box-primary">
            ...
            <div class="box-body no-padding">
              <div class="mailbox-controls">
                ...
                <!-- /.btn-group -->
                <a href="{{ URL::current()}}" type="button" class="btn btn-info btn-sm"><i class="fa fa-refresh"></i>&ensp;Refresh Inbox</a>
                ...
                <table class="table table-hover mailtable table-striped">
                  ...
                  <tbody>
                        ...
                        @foreach($inmate_numbers as $index =>$number)
                          @if($number->viewed == 1)
                              <tr class="boldemailtext">
                          @else 
                              <tr>
                          @endif 
                            <td>{{ ++$index }}</td>
                           
                            <td class="mailbox-name" ><a href="{{ route('viewchat', ['inmate_id'=> $inmate_id,'service_id' => $service_id,'number_id' => $number->id]) }}" style="text-decoration: none; border-bottom: 1px solid green;">{{ $number->name }}</a></td>
                            <td class="mailbox-subject" hidden ><b>{{ $number->email_phone }}</b></td>
