                                    ...
                                    @foreach($inboxmail as $index =>$mail)
                                     ...
                                     <tr class="boldemailtext">                                        <td >{{ ++$index }}</td>
                                            ...
                                            <td class="mailbox-name" >
                                                ...
                                                <a href="{{ route('inmate.generalemailview', ['inmate_id'=> $inmate_id,'service_id' => $service_id,'mail_id' => $mail->id]) }}" style="text-decoration: none; border-bottom: 1px solid green;">{{ $mail->name }}</a>
                                                @else
                                                <a href="{{ route('inmate.inboxemailview', ['inmate_id'=> $inmate_id,'service_id' => $service_id,'mail_id' => $mail->id]) }}" style="text-decoration: none; border-bottom: 1px solid green;">{{ $mail->name }}</a>@endif</td>
                                            <td class="mailbox-subject" ><b>{{ $mail->subject }}</b></td>
                                            <td class="mailbox-date">{{ Carbon\Carbon::parse($mail->created_at)->format('m/d/Y H:i:s') }}</td>
                                        </tr>
                                        @else
                                         <tr>
                                            <td >{{ ++$index }}</td>
